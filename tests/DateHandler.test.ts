...
import DateHandler from '../src/classes/DateHandler.ts'; // Importa la clase
...
describe('DateHandler Tests', () => {
  ...
  describe('Package.json', () => {
    describe('Date Generation for package.json', () => {
      let fakeDate: Date;

      beforeEach(() => {
        // Configura una fecha fija antes de cada test
        fakeDate = new Date(2026, 8, 11, 14, 30, 45, 123); // Septiembre 11, 2026, 14:30:45.123
        vi.useFakeTimers(); // Activa temporizadores falsos
        vi.setSystemTime(fakeDate); // Establece la fecha falsa como la hora del sistema
      });
      ...
      it('Should return a date in a compatible format: "January 15, 2025"', () => {
        expect(DateHandler.docs_date('2025-1-15')).toBe('January 15, 2025');
      });
      it('Should return a date in a compatible format: "September 11, 2026"', () => {
        expect(DateHandler.docs_date()).toBe('September 11, 2026');
      });
    });
  });
});
